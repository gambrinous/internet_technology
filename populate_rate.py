...
def populate():
    ...
    for i in range(len(universities)):
        ...
        #print u
        #add_university(universities[i][0], universities[i][1], universities[i][2], universities[i][3], universities[i][4], universities[i][5])

    c = conn.cursor()
    for row in c.execute('SELECT id FROM rate_university'):
        for j in range(len(students)):
                s = add_student(students[j][0], students[j][1], students[j][0]+'.'+students[j][1]+'@'+universities[i][1], '1234', University(int(row[0])))
        #you = University(row)
        #print you
        print int(row[0])#.decode('utf-8')

'''    for i in range(University.objects.count()):
        #you = University(University.objects.values('domain'))
        you = University()
        print "you: "
        print you

    print "Values_list: "
    print University.objects.values_list()
    print "Values: "
    print University.objects.values('id')
    #print "Order: "
    #print University.objects.
...
def add_uniCourse(uni, course, school, year, professor, rating):
    uC = UniCourse.objects.get_or_create(uni=uni, course=course, school=school, year=year, professor=professor, rating=rating)[0]
    return uC

def add_rate(student, course, rate, comment, date):
    r = Rating.objects.get_or_create(student=student, course=course, rate=rate, comment=comment, date=date)[0]
    return r
